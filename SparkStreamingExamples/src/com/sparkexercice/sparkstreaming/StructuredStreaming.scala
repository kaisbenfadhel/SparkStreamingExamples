...
object StructuredStreaming {
   ...
   def main(args: Array[String]) {
      ...
      val spark = SparkSession
        ...
        .master("local[*]")
        .config("spark.sql.warehouse.dir", "file:///D:/tmp") // Necessary to work around a Windows bug in Spark 2.0.0; omit if you're not on Windows.
        .config("spark.sql.streaming.checkpointLocation", "file:///D:/checkpoint")
        .getOrCreate()
        
      setupLogging()
        
      // Create a stream of text files dumped into the logs directory
      val rawData = spark.readStream.text("file:///D:/LearningSpark-master/logs")
